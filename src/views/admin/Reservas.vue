        ...
        <v-row>
          ...
          <v-col cols="12" sm="6" md="6" lg="3" xl="6" class="pt-0">
            <v-form ref="form">
              ...
              <template>
  <div>
    <v-dialog v-model="dialogFechaHora" persistent max-width="290px">
      <template v-slot:activator="{ on }">
        <v-text-field
          ...
          dark
          label="Fecha y Hora *"
          hint="Ingrese aquí la fecha y hora"
          :rules="[(v) => !!v || 'Este campo es requerido']"
          readonly
          v-on="on"
        ></v-text-field>
      ...
      <v-time-picker
        ...
        no-title
      ></v-time-picker>
      <v-btn text color="primary" @click="dialogFechaHora = false">Aceptar</v-btn>
    </v-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    listarReservas() {
      ...
      fetch(process.env.VUE_APP_API_URL + "Reserva")
        .then((resp) => resp.json())
        .then((data) => {
